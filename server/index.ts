...
import {
  ...
  type SessionPlayer,
  type SessionRevealResult,
  type SessionRoundView,
  type SessionSnapshot,
  type SessionPhase,
} from '../shared/multiplayer.js'
import { roundPack, type RoundContent } from '../shared/rounds.js'
import { scoreRound, type Coordinates } from '../shared/scoring.js'

type StoredSubmission = RoundSubmission
...
type StoredPlayer = {
  ...
  joinedAt: number
  totalScore: number
  roundResults: SessionRevealResult[]
}
...
app.post('/api/sessions', (request, response) => {
  ...
  const payload: SessionAuthResponse = { code, playerId }
...
app.post('/api/sessions/:code/join', (request, response) => {
  ...
  const body = request.body as JoinSessionRequest
  const name = sanitizeName(body?.name)
...
io.use((socket, next) => {
  const { code, playerId } = socket.handshake.auth as { code?: string; playerId?: string }
...
io.on('connection', (socket) => {
  ...
  socket.on('round:submit', (payload: RoundSubmission) => {
    const currentSession = sessions.get(code)
...
function resolveRound(session: StoredSession) {
  ...
  const revealResults = sortPlayers(session.players).map<SessionRevealResult>((player) => {
    const submission = session.submissions.get(player.id)

    if (!submission) {
      const emptyResult: SessionRevealResult = {
        playerId: player.id,
    ...
    const roundResult: SessionRevealResult = {
      playerId: player.id,
      playerName: player.name,
...
function emitSnapshot(session: StoredSession) {
  io.to(session.code).emit('session:snapshot', buildSnapshot(session))
}

function buildSnapshot(session: StoredSession): SessionSnapshot {
...
function buildRoundView(round: RoundContent, revealVisible: boolean): SessionRoundView {
  return {
    id: round.id,
    title: round.title,
    hint: round.hint,
...
function buildPlayerSummary(session: StoredSession, playerId: string): SessionPlayer {
  const player = session.players.get(playerId)
...
function getCurrentRound(session: StoredSession) {
  return session.rounds[session.roundIndex] ?? null
}

function sortPlayers(players: Map<string, StoredPlayer>) {
...
function isValidSituationIndex(value: number | undefined, round: RoundContent) {
